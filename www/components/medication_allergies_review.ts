 ...
 */
export default class TMedAllergiesReviewAppView extends TItemCardReviewAppView<UserAllergyAnswersArray> {

    constructor(aCtrl: TCtrl, opts?: AllergyReviewOptions) {
    ...
    public renderCurrentItem(currentItemIndex : number, direction: 'next' | 'prev' | null = null): void {
        ...
        const currentAllergy : UserAllergyAnswers = this.itemData[currentItemIndex];
        ...
        const replayRadios = newCard.querySelectorAll<HTMLInputElement>(`input[name="allergic_${currentItemIndex}"]`);
    ...
    public captureItemAnswer(target: HTMLInputElement | HTMLTextAreaElement): void {
        const currentAllergy : UserAllergyAnswers = this.itemData[this.currentItemIndex];
        ...
        if (target.name.startsWith('allergic_')) {
            if (['yes', 'no', 'unknown'].includes(target.value) == true) {
                currentAllergy.patientResponse = value as YesNoUnknownStatus;
            }
        } else if (target.name.startsWith('comment_')) {
            currentAllergy.comment = (target as HTMLTextAreaElement).value.trim();
        }
    }
    ...
    public isItemAnsweredComplete(itemIndex: number, forceRecalc: boolean = false): boolean { // Changed parameter to medicationIndex
        ...
        if (!currentAllergy) return false;
