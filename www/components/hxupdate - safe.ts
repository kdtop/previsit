...
/**
 * Defines the shape of the JSON response from the `/api/hxupdate` endpoint.
 */
interface HxUpdateApiResponse {
    success: boolean;
    html: string;
    message?: string;
}
...
export default class THxUpdateAppView extends TAppView {
    ...
    private async loadForms(): Promise<void>
    {
        ...
        const data: HxUpdateApiResponse = await response.json();
        if (data.success) {
            ...
            const tempEl: HTMLDivElement = document.createElement('div');
            ...
            checkboxes.forEach(checkbox => {
                const htmlCheckbox = checkbox as HTMLInputElement;  // Cast checkbox to HTMLInputElement to access .dataset and .checked properties
                function toggleVisibility(isChecked: boolean): void
                {
                    // Get the comma-separated string of IDs to hide/show
                    const targetIdsString: string | undefined = htmlCheckbox.dataset.hideTargetIds;
                    if (!targetIdsString) return;
                    const targetIds: string[] = targetIdsString.split(','); // Split into an array of IDs
                    targetIds.forEach((id: string) => {
                        const targetElement: HTMLElement | null = document.getElementById(id.trim()); // Trim whitespace
                        if (targetElement) {
                            if (isChecked) {
                                targetElement.classList.add('hidden'); // Hide the element
                                // If the element contains checkboxes or a textarea, clear/uncheck them
                                const checkboxesInTarget: NodeListOf<HTMLInputElement> = targetElement.querySelectorAll('input[type=\'checkbox\']');
                                checkboxesInTarget.forEach((cb: HTMLInputElement) => {
                                    cb.checked = false; // Uncheck all checkboxes within the hidden section
                                });
                                const textareaInTarget: HTMLTextAreaElement | null = targetElement.querySelector('textarea');
                                if (textareaInTarget) {
                ...
                htmlCheckbox.addEventListener('change', function(this: HTMLInputElement) { // Explicitly type 'this' for the event listener
