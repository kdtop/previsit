...
import { ToggleButton,
        } from './comp_btns.js'; // Import both components
...
export default class TPatientConsentFormAppView extends TAppView<ConsentFormData> {
    ...
    getHTMLAgreeToggle(name : string) {
        let result : string = `
    ...
    private getHTMLSectionA() : string {
        let result : string = `
    ...
    private getHTMLSectionB() : string {
        let result : string = `
            <div class="form-section">
                <div class="form-section-header">
    ...
    private getHTMLSectionC() : string {
        let result : string = `
            <div class="form-section">
                <div class="form-section-header">
    ...
    private getHTMLSectionD() : string {
        let result : string = `
            <div class="form-section">
                <div class="form-section-header">
    ...
    public setupFormEventListeners(): void {
        ...
        const previewBtn = this.htmlEl.dom.getElementById("previewBtn") as HTMLButtonElement;
        if (previewBtn) previewBtn.addEventListener("click", () => this.handlePreviewClick());
    ...
    private getSigAreaVisibility() : boolean {
        if (!this.htmlEl.signatureSection) return false;
    ...
    public updateDoneButtonState(): void
    {
        ...
        let signed : boolean = this.getIsSigned();

        let dontSignBtn = this.htmlEl.dom.querySelector<HTMLButtonElement>('.dont-sign-btn');
        if (dontSignBtn) {
    ...
    public gatherDataForServer = (): ConsentFormData => {
        ...
        let signed : boolean = this.getIsSigned();
        let inactiveFormHTML : string = signed ? this.getFrozenHTML() : '';
     ...
     */
    public getFrozenHTML(): string {

        // 1. Grab the <form> inside the shadow root
        let formEl: HTMLFormElement | null = this.htmlEl.dom.querySelector("form");
        if (!formEl) throw new Error("No form element found in shadow root");

        // 2. Clone the form so we don't mutate the live DOM (so the user can still edit the live version later)
        const frozen = formEl.cloneNode(true) as HTMLElement;

        let toggleBtnCSS : string = "";

        let toggleInputs: NodeListOf<ToggleButton> = frozen.querySelectorAll<ToggleButton>("toggle-button");
        toggleInputs.forEach((el : ToggleButton) => {
            ...
            let textEl : HTMLElement | null | undefined = el.shadowRoot?.querySelector(".label-text-wrapper");
            let text : string = textEl?.textContent || "";  //get text of label-text-wrapper element.
            let wrapper = document.createElement("div");
            let html = el.getHTMLTagContentForInactiveDisplayOnly().trim();
            wrapper.innerHTML = html;
            let replacement : Element | null = wrapper.firstElementChild;
            if (!replacement) return;
            el.replaceWith(replacement);
            let newTextEl = replacement.querySelector(".label-text-wrapper"); //look for label-text-wrapper element from new segment.
            if (newTextEl) newTextEl.textContent = text;  //put text from prior ToggleButton into new frozen html.
        });

        let htmlElements : NodeListOf<HTMLDivElement> = frozen.querySelectorAll<HTMLDivElement>(".form-actions");
        htmlElements.forEach((el : HTMLDivElement) => {
            el.remove();
        });

        // Replace inputs
        type HTMLInputs = HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
        let inputs : NodeListOf<HTMLInputs> = frozen.querySelectorAll<HTMLInputs>("input, textarea, select");
        inputs.forEach(el => {
          ...
          if (el instanceof HTMLInputElement && (el.type === "checkbox" || el.type === "radio")) {
            ...
            replacement = document.createElement("span");
            replacement.textContent = (el as HTMLInputElement | HTMLTextAreaElement).value;
            replacement.classList.add("inactive-input");
          }
        ...
        const signaturePad: SignaturePadComponent | null | undefined = this.htmlEl.signaturePadComponent
        const sigAreaDiv = frozen.querySelector<HTMLDivElement>(".signature-area");
