...
import { showPopupDlg, ModalBtn } from './dialog_popup.js';
// ---------------------------------------------------
...
export default class TDashboardAppView extends TAppView {
    ...
    handleDoneClick = async () => {
        ...
        let submitConfirmed = await this.confirmSubmit();
        if (!submitConfirmed)
            return;
    ...
    confirmSubmit = async () => {
        ...
        let unansweredForms = this.progressData.unansweredItems || 0;
        let instructions = (unansweredForms > 0) ? `NOTE: ${unansweredForms} Forms are INCOMPLETE!`
            : "All forms COMPLETED";
        const schema = {
        ...
        const result = (modalResult == ModalBtn.OK);
        return result;
    };
}
