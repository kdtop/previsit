...
async function hndlSaveDashboard(req, res) {
    ...
    try {
        ...
        let oneResponse = data[0]; //In dashboard, in gatherDataForServer(), one record to hold signal is created.
        if (oneResponse && oneResponse?.formsHaveBeenSubmitted === true) {
            let err = "";
            let errIndex = 1; // Output parameter for errors from Mumps (1st param is #0)
            let tag = "FINALIZEFORMS";
            let rtn = "TMGPRE01";
            const rpcResult = await tmg.RPC(tag, rtn, [sessionID, err]); //<-- no data sent.  Just calling function to finalize forms.
            if (!rpcErrorCheckOK(rpcResult, res, errIndex, tag, rtn))
                return; //if rpcErrorCheckOK returns false, res.status will have been set with error...
